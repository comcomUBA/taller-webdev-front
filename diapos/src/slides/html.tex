...
\begin{frame}[fragile]
    ...
    Las tags tienen el formato \inlinecode{<tag atributos>Contenido</tag>} o \\
    \inlinecode{<tag atributos/>} para elementos sin contenido. Dicho contenido 
...
\begin{frame}[fragile]
    ...
    Las tags más importante que cualquier sitio debe tener son \inlinecode{html}, 
    \inlinecode{head} y \inlinecode{body}. La primera es la que define los límites del documento, 
    ...
    sitio. Por último, \inlinecode{body} es donde va el contenido del sitio, básicamente todo.
...
\begin{frame}[fragile]
    \frametitle{Contenidos de \inlinecode{head}, metadatos}
    
    \begin{itemize}
        \item \inlinecode{title} se usa para elegir el título de la página. Dicho 
              título es el que se ve en la pestaña correspondiente del navegador.
        \item \inlinecode{link} define relaciones con recursos externos al documento. 
              ...
              lado del título). En el caso más general usa el atributo 
              \inlinecode{href} para definir la dirección del recurso y \inlinecode{rel} para 
              especificar cual es dicha relación.
        \item \inlinecode{style} permite agregar estilo directamente ahí pero no es muy 
              común hacer eso.
        \item \inlinecode{meta} sirve para agregar cualquier otro tipo de información 
...
\begin{frame}[fragile]
    ...
    HTML viene con los elementos \inlinecode{header}, \inlinecode{main} y \inlinecode{footer} para 
    organizar el sitio en esas tres partes. También incluye cosas como \inlinecode{aside} 
    para cosas que justamente deberían ir a un costado, \inlinecode{article} para artículos, 
    etc. (todas bastante auto explicativas).
    
    Sin embargo, suele pasar (y es discutible si es algo bueno o malo) que el único 
    elemento que se termina usando para organizar el sitio es \inlinecode{div}. El elemento 
...
\begin{frame}[fragile]
    ...
    Los elementos \inlinecode{hN}, donde la $N$ va de 1 a 6, son encabezados. La importancia 
    va de menor a mayor, donde \inlinecode{h1} es el título principal del documento (y debería 
    haber solo uno) y \inlinecode{h6} es un título bastante menor.

    El elemento \inlinecode{p} representa un párrafo, aunque se puede meter todo el texto que 
    quieran ahí adentro.

    Esos son los principales elementos que contienen texto y ambos tienen formato block.
    Pero también hay elementos que modifican el texto que está adentro de los \inlinecode{p} 
    y \inlinecode{h}.

    \inlinecode{a} es para agregar links, \inlinecode{strong} es para marcar importancia, 
    \inlinecode{em} es para énfasis, etc. Al igual que \inlinecode{div}, la versión genérica 
    de los modificadores es \inlinecode{span}. Todos estos elementos tienen formato 
...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        \item \inlinecode{ul} es para listas no ordenadas (el punteado no tiene números) y 
              \inlinecode{ol} es para listas ordenadas (el punteado si tiene números (o letras
              (o lo que quieran (se puede cambiar todo)))). Los elementos que van adentro 
              de estas listas deberían ser \inlinecode{li}.
        \item \inlinecode{table} es para tablas. Hay un montón de elementos relacionados a las 
              tablas para organizarlas como quieran, pero los más importantes son 
              \inlinecode{tr} para filas, \inlinecode{th} para cabezales de columnas y \inlinecode{td}
              para celdas.
    \end{itemize}
...
\begin{frame}[fragile]
    ...
    \begin{itemize}
        \item Para usar imágenes está \inlinecode{img}. El atributo \inlinecode{src} define la dirección
              de la imagen a mostrar. Tiene unos cuantos atributos más para cambiar como 
              aparece y tocar temas de accesibilidad.
        \item Los formularios son con \inlinecode{form} y los campos de dichos formularios 
              son con \inlinecode{input}. El atributo \inlinecode{type} define que tipo de 
              campo es.
    \end{itemize}
...
\begin{frame}[fragile]
    ...
    También podemos escribir comentarios con una sintaxis muy similar:

\begin{lstlisting}
<!-- Esto es un comentario -->

<!--
    Todo lo que esté entre esas tags no es leído por el navegador
    <p>No importa que haya adentro</p>
-->
\end{lstlisting}
